...
class Database
{
    ...
    {
        if (!($this->_db = mysql_connect($hostname, $username, $password))) {
            throw new Exception('Unable to connect to database.');
        }

        if (!mysql_select_db($database, $this->getDb())) {
            throw new Exception('Unable to select the scheme.');
        }
    }
    ...
    public function query ($query)
    {
        ...
        if ($result === false) {
            throw new Exception('Unable to execute the query. ' . mysql_error($this->getDb()));
            ...
            $affectedRows = mysql_affected_rows($this->getDb());
...
abstract class Exporter extends Database
{
    ...
    protected $_columns = array(
        ...
        'ticket_logs' => array ( 'id', 'tid', 'userid', 'date', 'message', 'email'),
...
class WHMCS_Exporter extends Exporter
{
    ...
    public function exportDomains ()
    {
        ...
        do {
            $query = "SELECT userid, registrationdate, domain, recurringamount, registrationperiod, status, nextinvoicedate FROM tbldomains LIMIT {$offset}, {$this->_mysqlBufferLimit}";
    ...
    public function exportPackages ()
    {
        ...
        do {
            $query = "SELECT pro.id, pro.gid, pro.name, pri.monthly, pri.quarterly, pri.semiannually, pri.annually, pri.biennially, pro.description, pro.type, pro.tax, pri.msetupfee FROM tblproducts pro, tblpricing pri WHERE pri.currency =1 AND pro.id = pri.relid AND pri.type = 'product' AND pri.monthly <> '-1' LIMIT {$offset}, {$this->_mysqlBufferLimit}";
            $result = $this->query($query);
