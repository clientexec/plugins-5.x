...
class Predicted_Income extends Report
{
    ...
    function process()
    {
        ...
        for($monthsAhead = 0; $monthsAhead < $amountOfMonths; $monthsAhead++){
            $newMonthTotals[date("Y M", mktime(0, 0, 0, date("m") + $monthsAhead, date("d"), date("Y")))] = array(
                'amount' => 0,
                'taxes'  => 0,
                'total'  => 0
            );
        ...
        while(list($billdate, $amount, $subtotal) = $result2->fetch()) {
            $monthAndYear = date("Y M", mktime(0, 0, 0, date("m", strtotime($billdate)), date("d", strtotime($billdate)), date("Y", strtotime($billdate))));
        ...
        $maxMonthAndYear = date("Ym", mktime(0, 0, 0, date("m", time()) + $amountOfMonths - 1, date("d", time()), date("Y", time())));

        //Add recurring charges
        while(list($info) = each($masterGroup)) {
            $nextbilldate    = $masterGroup[$info][0];
            $amount          = $masterGroup[$info][1];
            $taxes           = $masterGroup[$info][2];
            $paymentterm     = $masterGroup[$info][3];

            $monthsAhead = 0;
            $currentMonthAndYear = 0;
            do{
                $monthAndYear = date("Y M", mktime(0, 0, 0, date("m", strtotime($nextbilldate)) + $monthsAhead, date("d", strtotime($nextbilldate)), date("Y", strtotime($nextbilldate))));
                if(isset($newMonthTotals[$monthAndYear])){
                ...
                $currentMonthAndYear = date("Ym", mktime(0, 0, 0, date("m", strtotime($nextbilldate)) + $monthsAhead, date("d", strtotime($nextbilldate)), date("Y", strtotime($nextbilldate))));
        ...
        $MonthsToDisplay =
            ...
            .'        Months to display: '
            .'        <input type="text" name="amountOfMonths" id="amountOfMonths" size="2" value="'.$amountOfMonths.'"> '
