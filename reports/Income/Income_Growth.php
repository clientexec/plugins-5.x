...
class Income_Growth extends Report {
    ...
    function process() {
        ...
        $url = "admin/index.php?noheaderfooter=1&getinvoices=1&month='+month+'&year='+year+'&view=".$_REQUEST['view']."&fuse=".$_REQUEST['fuse']."&report=".urlencode($_REQUEST['report'])."&type=".$_REQUEST['type'];
        $url = mb_substr($params['clientExecURL'],-1,1) == "//" ? $params['clientExecURL'].$url : $params['clientExecURL']."/".$url;
        echo "    url ='".$url."';\n";
    ...
    function GetInvoiceForMonth() {
        ...
        echo "<H1>" . $tMonth . " " . $nCurrentYear . "</h1></br>";
        ...
        while (list($nInvoiceId, $userID, $dAmount, $dSubtotal, $dTax, $dtDatePaid, $strCurrency, $taxRate) = $result->fetch()) {
            ...
            $CustomerLink = "<a href=\"index.php?fuse=clients&controller=userprofile&frmClientID=" . $userID . "&view=profileinvoices\" $title target=blank>" . $dCustomerName . "</a>";
        ...
        if ($monthInvoices > 0){
            ...
            echo "<tr><td>" . $totalInvoices . "</td><td>" . $currency->format($strBaseCurrency, $incomeTotal, true) . "</td><td>" .  $currency->format($strBaseCurrency, $TotalTax, true) . "</td><td>" .  $currency->format($strBaseCurrency, $incomeTotal - $TotalTax, true) . "</td></tr>";
            echo "</table>";
        }
    }
